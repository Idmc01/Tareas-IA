...
class LitDistilledClassifier(pl.LightningModule):
    ...
    def distillation_loss(self, student_logits, teacher_logits, labels):
        ...
        distill_loss = self.kl_loss(soft_predictions, soft_targets) * (self.temperature ** 2)
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("train/hard_loss", hard_loss, on_step=False, on_epoch=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val/distill_loss", distill_loss, on_step=False, on_epoch=True)
        self.log("val/hard_loss", hard_loss, on_step=False, on_epoch=True)
        self.log("val/acc", acc, prog_bar=True, on_step=False, on_epoch=True)
        
        self.validation_outputs.append({
            'logits': student_logits.detach().cpu(),
            'probs': probs.detach().cpu(),
            'preds': preds.cpu(),
            'labels': labels.cpu(),
            'features': student_features.detach().cpu(),
            'defects': defects
        })
    ...
    def _plot_confusion_matrix(self, preds, labels, split_name):
        ...
        ax.set_ylabel('True')
        ax.set_title(f'Confusion Matrix - Modelo B Distilled ({split_name})')
        plt.tight_layout()
        return fig
    ...
    def _plot_tsne(self, features, labels, defects, split_name):
        ...
        ax1.set_title(f't-SNE Modelo B ({split_name}): Good vs Defect')
